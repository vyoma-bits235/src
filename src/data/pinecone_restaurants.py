...
def process_restaurants():
    ...
    restaurant_data = [{"id": r.id, "name": r.name, "slug": r.slug,"price":r.price} for r in restaurants]
    ...
    df = df.groupby("id").agg({"name": "first", "slug": "first", "food_text": "sum"}).reset_index()
    
    # Creating text for embedding
    df["text"] = "```" + df["name"] + "\nRestaurant description: " + df["slug"] + "\nFood available:" + df["food_text"] + "\n```"
    ...
    documents = [
        Document(
            ...
            metadata={
                ...
                "restaurant_description": df_row[1]["slug"],
